...
impl AppState {
    ...
    fn remove_selected(&mut self) {
        ...
        if let Some(id) = self.selected_job_id(&jobs) {
            if let Ok(mut q) = self.queue.lock() {
                q.remove(id);
                self.message = Some("Job removed.".to_string());
            }
            if let Some(sel) = self.list_state.selected() {
                ...
                if len <= 1 {
                    ...
                    self.list_state.select(Some((sel + len - 1) % (len - 1).max(1)));
                }
            }
        } else {
            self.message = Some("No job selected.".to_string());
        }
    }

    fn submit_add_task(&mut self, form: &AddTaskForm) {
        ...
        if time_str.is_empty() || priority_str.is_empty() || desc.is_empty() || func.is_empty() {
            self.message = Some("All fields required.".to_string());
            return;
        }

        let execution_time: i64 = match time_str.parse() {
            Ok(t) => t,
            Err(_) => {
                self.message = Some("Invalid timestamp (use Unix seconds).".to_string());
                return;
            }
        };
        let priority: u8 = match priority_str.parse() {
            Ok(p) => p,
            Err(_) => {
                self.message = Some("Priority must be 0–255.".to_string());
                return;
            }
        };
        ...
        match Job::new(execution_time, priority, desc, func) {
            Ok(job) => {
                if let Ok(mut q) = self.queue.lock() {
                    q.push(job);
                    self.message = Some("Job added.".to_string());
                }
...
pub fn run_tui(
    ...
    let mut app = AppState::new(queue, log_rx, worker_tx);
...
fn ui(f: &mut Frame, app: &mut AppState, _form: &AddTaskForm) {
    ...
    let list_items: Vec<ListItem> = jobs
        .iter()
        .map(|j| {
            let ts = Utc.timestamp_opt(j.execution_time, 0)
                .single()
    ...
    let log = Paragraph::new(log_text)
        .block(Block::default().title(" Engine / Worker log ").borders(Borders::ALL))
        .wrap(Wrap { trim: true })
        .style(Style::default().fg(Color::DarkGray));
    f.render_widget(log, main_chunks[1]);

    let help = match app.input_mode {
        InputMode::Normal => " Ctrl+A: Add task │ D: Delete selected │ Q: Quit ",
        InputMode::AddTask => " Enter: Next field │ Esc: Cancel │ Time = Unix timestamp (seconds) ",
    };
